...
import {shopConfirmOrder} from "../api/products";

const phoneRegExp = /^[\+]?[(]?[0-9]{3}[)]?[-\s\.]?[0-9]{3}[-\s\.]?[0-9]{4,6}$/

const OrderForm = ({setUserName, init}) => {
    ...
    const onSubmit = (event) => {
        if (init === 1){
            dispatch(shopConfirmOrder(user, event.first_name, event.last_name, event.phone, event.email, event.city, event.comments, delivery, order))
        }else {
